...
先発列車は本線側に停車＆出発、後発列車は待避側で停車＆出発
2回目の停車で揃います。
TNOS の 1-1 エンドレス 交互運転 の動作となります。
...
tr2 = Train(33, 550, 200, "horn2.mp3")
tr1 = Train(70, 450, 200, "horn1.mp3")
...
def train_secondery_move(train: Train):
    ds.waitForS88(1, S88.round_list[S88.late_train_start_index])                 # 先行列車が進出検出位置に到達
    ds.setTurnout(1,1)                                  # 出発ポイントを退避側へ
    playsound.playsound(train.horn_file, False)
    train_go_around(train, 2, 1)
    train_stop_station(train, S88.t1_slow, S88.t1_stop) # 直線側に停車
    time.sleep(1.0)
    ds.waitForS88(1, S88.round_list[S88.late_train_start_index])                 # 先行列車が進出検出位置に到達
    ds.setTurnout(1,1)                                  # 出発ポイントを退避側へ
    playsound.playsound(train.horn_file, False)
